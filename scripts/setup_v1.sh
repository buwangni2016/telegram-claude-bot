#!/bin/bash
# ================================================
# setup_v1.sh - 极简内核版部署脚本 (v1-simple)
# 用法: bash scripts/setup_v1.sh <BOT_TOKEN>
# ================================================
set -e
...
BOT_SCRIPT="$REPO_DIR/bot_v1.py"
CONFIG="$REPO_DIR/config/v1.json"
...
echo ""
echo "================================================"
echo "  Telegram Claude Bot v${VERSION} (v1-simple)"
echo "================================================"
echo ""
...
[ -z "$BOT_TOKEN" ] && err "请提供 Token: bash scripts/setup_v1.sh <TOKEN>"
...
try:
    ...
    print(f'ERROR:{e}')
" 2>/dev/null)
echo "$BOT_INFO" | grep -q "ERROR" && err "Token 无效: $BOT_INFO"
ok "Bot 验证: $BOT_INFO"
...
echo "  bash scripts/setup_v1.sh   # 重新部署"
echo ""
echo -e "${YELLOW}升级提示:${NC}"
echo "  v2-secure: git checkout v2-secure && bash scripts/setup_v2.sh"
echo "  v3-full:   git checkout v3-full   && bash scripts/setup_v3.sh"
echo ""
